...
где `{folder}` — подпапка, вычисляемая через `shopProduct::getFolder($product_id)`. Старый стиль — только унаследованные данные, созданные до 3.0.0; заново он не создаётся никогда. `shopSyrattachFileModel::add()` безусловно пишет `product_id => null`, поэтому в центральное хранилище идут и загрузки из редактора товара, и файлы, прикреплённые при импорте из CSV. При удалении товара Shop-Script сам зачищает его директорию (и старые файлы); новые файлы без оставшихся ссылок плагин удаляет сам.

Каждый файл лежит в собственной директории, названной по его `id`, и она создаётся заново под только что вставленную запись. Коллизий имён поэтому не бывает: одноимённые файлы сосуществуют, ничего не переименовывается и не перезаписывается.
